...
jhhrvstdsalaryUI <- function(tabTitle ='工资',
...
buttonsalaryUI_left <- function() {
  ...
  res <- tagList(
    ...
    shiny::actionButton(inputId = 'btn_hrv_std_view_salary' , label = '预览表单数据'),
...
buttonsalaryUI_right <- function() {
  res <- tagList(
    tsui::uiScrollX(tsui::mdl_dataTable(id = 'hrv_std_view_data_salary', label = '出口'))
  )
  return(res)
